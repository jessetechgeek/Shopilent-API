...
public class S3StorageServiceTests : IntegrationTestBase
{
    ...
    private async Task EnsureTestBucketExists()
    {
        ...
        try
        {
            var existsResult = await _s3StorageService.ListFilesAsync(_testBucket);
    ...
    public async Task UploadFileAsync_WithValidFile_ShouldReturnSuccessWithUrl()
    {
        ...
        // Act
        var result = await _s3StorageService.UploadFileAsync(_testBucket, key, stream, contentType);
    ...
    public async Task UploadFileAsync_WithMetadata_ShouldReturnSuccessWithUrl()
    {
        ...
        // Act
        var result = await _s3StorageService.UploadFileAsync(_testBucket, key, stream, contentType, metadata);
    ...
    public async Task DownloadFileAsync_WithExistingFile_ShouldReturnStreamWithContent()
    {
        ...
        var uploadResult = await _s3StorageService.UploadFileAsync(_testBucket, key, uploadStream, "text/plain");
        ...
        // Act
        var downloadResult = await _s3StorageService.DownloadFileAsync(_testBucket, key);
    ...
    public async Task DownloadFileAsync_WithNonExistentFile_ShouldReturnNotFoundError()
    {
        ...
        // Act
        var result = await _s3StorageService.DownloadFileAsync(_testBucket, key);
    ...
    public async Task DeleteFileAsync_WithExistingFile_ShouldReturnSuccess()
    {
        ...
        // First upload the file
        var uploadResult = await _s3StorageService.UploadFileAsync(_testBucket, key, stream, "text/plain");
        uploadResult.IsSuccess.Should().BeTrue();

        // Act
        var deleteResult = await _s3StorageService.DeleteFileAsync(_testBucket, key);
        ...
        var existsResult = await _s3StorageService.FileExistsAsync(_testBucket, key);
    ...
    public async Task DeleteFileAsync_WithNonExistentFile_ShouldReturnSuccess()
    {
        ...
        // Act
        var result = await _s3StorageService.DeleteFileAsync(_testBucket, key);
    ...
    public async Task FileExistsAsync_WithExistingFile_ShouldReturnTrue()
    {
        ...
        // Act
        var result = await _s3StorageService.FileExistsAsync(_testBucket, key);
    ...
    public async Task GetPresignedUrlAsync_WithValidFile_ShouldReturnValidUrl()
    {
        ...
        // Act
        var result = await _s3StorageService.GetPresignedUrlAsync(_testBucket, key, expiry);
    ...
    public async Task ListFilesAsync_WithEmptyBucket_ShouldReturnEmptyList()
    {
        ...
        // Act
        var result = await _s3StorageService.ListFilesAsync(_testBucket, prefix);
    ...
    public async Task ListFilesAsync_WithMultipleFiles_ShouldReturnAllFiles()
    {
        ...
        foreach (var file in files)
        {
            ...
            var contentBytes = Encoding.UTF8.GetBytes(content);
            using var stream = new MemoryStream(contentBytes);
            var uploadResult = await _s3StorageService.UploadFileAsync(_testBucket, file, stream, "text/plain");
            uploadResult.IsSuccess.Should().BeTrue();
        }
    ...
    public async Task ListFilesAsync_WithPrefix_ShouldReturnFilteredFiles()
    {
        ...
        {
            $"{targetPrefix}/file1.txt",
            $"{targetPrefix}/file2.txt"
        };
        
        var otherFiles = new List<string>
        {
            $"{otherPrefix}/file3.txt"
        };
        ...
        // Act
        var result = await _s3StorageService.ListFilesAsync(_testBucket, targetPrefix);
    ...
    public async Task ListFilesAsync_WithoutPrefix_ShouldReturnAllFilesInBucket()
    {
        ...
        // Act
        var result = await _s3StorageService.ListFilesAsync(_testBucket);
        ...
        try
        {
            ...
            switch (provider)
            {
                ...
                case "AWS":
                    ...
                    break;
                default: // MinIO
                    serviceUrlProperty?.SetValue(_s3Settings, IntegrationTestFixture.MinioContainer.GetConnectionString());
                    break;
            }

            // Act
            var result = await _s3StorageService.UploadFileAsync(_testBucket, key, stream, "text/plain");
    ...
    [Fact]
    public async Task UploadFileAsync_WithInvalidBucket_ShouldReturnFailure()
    {
        // Arrange
        var invalidBucket = "invalid-bucket-name-that-does-not-exist";
        var key = $"test-files/{Guid.NewGuid()}.txt";
        var content = "Hello, Invalid Bucket Test!";
        var contentBytes = Encoding.UTF8.GetBytes(content);
        using var stream = new MemoryStream(contentBytes);

        // Act
        var result = await _s3StorageService.UploadFileAsync(invalidBucket, key, stream, "text/plain");

        // Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Type.Should().Be(ErrorType.Failure);
        result.Error.Message.Should().Contain("Failed to upload file");
    }

    [Fact]
    public async Task DownloadFileAsync_WithInvalidBucket_ShouldReturnFailure()
    {
        // Arrange
        var invalidBucket = "invalid-bucket-name-that-does-not-exist";
        var key = $"test-files/{Guid.NewGuid()}.txt";
        ...
        // Act
        var result = await _s3StorageService.DownloadFileAsync(invalidBucket, key);

        // Assert
        result.IsFailure.Should().BeTrue();
        // Invalid bucket returns NotFound error in MinIO/S3
        result.Error.Type.Should().Be(ErrorType.NotFound);
        result.Error.Message.Should().Be("File not found");
    }
}