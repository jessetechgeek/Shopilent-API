...
internal sealed class GetPaymentMethodQueryHandlerV1 : IQueryHandler<GetPaymentMethodQueryV1, PaymentMethodDto>
{
    ...
    public async Task<Result<PaymentMethodDto>> Handle(GetPaymentMethodQueryV1 request, CancellationToken cancellationToken)
    {
        try
        {
            var paymentMethod = await _unitOfWork.PaymentMethodReader.GetByIdAsync(request.Id, cancellationToken);
            ...
            {
                _logger.LogWarning("User {UserId} attempted to access payment method {PaymentMethodId} belonging to user {OwnerId}", 
            ...
            _logger.LogInformation("Retrieved payment method with ID {PaymentMethodId} for user {UserId}", request.Id, request.UserId);
            return Result.Success(paymentMethod);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving payment method with ID {PaymentMethodId} for user {UserId}", request.Id, request.UserId);
